mrf_elm_forecast <- function(UnivariateData, Horizon, Aggregation,
  ...
  for(i in 1:NumLevels){
    currForecast = 0
    if(i < NumLevels){
      model        = nnfor::elm(y = stats::as.ts(dec_res$WaveletCoefficients[i,]))
      tmpForecast  = forecast::forecast(model,h=Horizon)
      currForecast = as.vector(tmpForecast$mean)
    }else{
      model        = nnfor::elm(y = as.ts(dec_res$SmoothCoefficients[i-1,]))
      tmpForecast  = forecast::forecast(model,h=Horizon)
      currForecast = as.vector(tmpForecast$mean)
    }
