...
plot(UnivariateData, type = "l", col = 4)
```
...
```{r, fig.dim = c(7, 4)}
dec = mrf::decomposition(UnivariateData = UnivariateData, Aggregation = c(2,4))
plot(dec$WaveletCoefficients[1,2:length(dec$SmoothCoefficients[1,])], type = "l", main = "Wavelet level 1", col = 4)
```

```{r, fig.dim = c(7, 4)}
plot(dec$WaveletCoefficients[2,4:length(dec$WaveletCoefficients[1,])], type = "l", main = "Wavelet level 2", col = 4)
```

```{r, fig.dim = c(7, 4)}
plot(dec$SmoothCoefficients[2,4:length(dec$SmoothCoefficients[1,])], type = "l", main = "Last smooth part level", col = 4)
```
...
```{r, eval = FALSE}
rw_forecasts = mrf::rolling_window(UnivariateData, CoefficientCombination,
...
```{r, eval = FALSE}
MAE = sum(abs(rw_forecasts$Error))/(3*2)
MAE
```
...
data = UnivariateData[1:(len_data-365)]
...
```{r, eval = FALSE}
nested_cross_validation(UnivariateData, Horizon = 14, EvaluationLength=2,
                        TestLength=2, Method="r", MultivariateData=NULL,
